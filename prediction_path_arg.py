...
import pickle
import numpy as np
from src.model import NIT_Registration
...
def predict_label(temp_pos, temp_label, test_pos, temp_color=None, test_color=None, cuda=False, topn=5):
    # EDITED BY ERIN - orig n_hidden = 128, n_layer = 6, p_rotate =0m feat_trans=0
    model = NIT_Registration(input_dim=3, n_hidden=128, n_layer=6, p_rotate=1, feat_trans=1, cuda=cuda)
    device = torch.device("cuda" if cuda else "cpu")
    # load trained model
    # ERIN EDIT ORIG "model_path = "../model/model.bin""
    #model_path = "model.bin"
    model_path='./model/late_embryo_final.bin'
    params = torch.load(model_path, map_location=lambda storage, loc: storage)
    #print("params from DNC pred: ", params.keys())
    #print("params dict: ", params)
    # changed from params['state_dict'] to just params by erin 14Aug
    ...
    #print("what's appended:",temp_pos[:,:3])
    ...
    with torch.no_grad():
        _, _, output_pairs = model(pt_batch, match_dict=None, ref_idx=data_batch['ref_i'], mode='eval')

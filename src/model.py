...
class neuron_data_pytorch(Dataset):
    ...
    """
    # show_name = False orig. but subbing for True to try to troubleshoot poor rotation pred. accuracy - Erin 28Jul
    def __init__(self, path, batch_sz, shuffle, rotate=False, mode='all', ref_idx=0, show_name=True, shuffle_pt = True, tmp_path= None):
        """
        ...
        # uniq name list added by erin for sanity check
        #unique_name_list=[]
        # set the temp_plate
    ...
    def load_path(self, path, batch_sz):
        ...
        :return:
        """
        # ANNOT BY ERIN 7/4 - will use mode "all" (good b/c using sim data only) - just finds files with these patterns
        if self.mode == 'all':
        ...
        bundle_list = list()
        num_data = 0
        # ANNOT BY ERIN 7/4 - looks for .npy files - but this doesn't contain indexing info?
        # iterate thru folders (for our use just train and eval folders bc .npy files all in one folder)
        for folder_idx, folder in enumerate(self.folders):
            if self.mode == 'all':
                volume_list = glob.glob1(folder, '*.npy')
                #print("vol list: ", volume_list)
            elif self.mode == 'real':
                volume_list = glob.glob1(folder, 'real_*.npy')
            elif self.mode == 'syn':
                volume_list = glob.glob1(folder, 'syn_*.npy')

            num_volume = len(volume_list) # num of .npy files
            num_data += num_volume  # keeps running list of how much data over all folders (all in one for us)
            # EDIT BY ERIN 9 Aug moving into else so this doesn't get messed up? or prob might not matter...
            all_files.append(volume_list)  #combines data into all_files list (all in one step for us)

            if batch_sz > num_volume:
                # ERIN UNCOMMENT? 7/5 - this allowed it to run but don't think it's correct?
                print("batch size is greater than num volumes!")
                #print("NUM VOLUMES: ",num_volume)
                #print("BATCH SIZE: ", batch_sz)
                # Uncommented 9 Aug to test multiple folder format
                #bundle_list.append([folder_idx, 0, num_volume]) # get more data? not sure about bundle_list (order of batch as described in shuffle_batch()?)
            else:
                #all_files.append(volume_list)
                # weird for loop/if statement block to iterate through data...
    ...
    def custom_collate_fn(self, bundle):
        ...
        for volume_idx in range(bundle[1], bundle[2]):
            ...
            if self.show_name:
                ...
                # ADDED BY ERIN 28JUL
                #print("pt_name_list: ",pt_name_list)
        ...
        for i in range(len(label_batch)):
            ...
            # EDITED BY ERIN commented out in Leifer repo - RECOMMENT?
        ...
        if self.show_name:
            ...
            # added by erin to append unique names to overall list
            uniq_names = list(set(list(pt_name_list)) - set(unique_name_list))
            unique_name_list.append(uniq_names)
        #print("UNIQUE NAMES: ",unique_name_list)
        #print("LENGTH OF UNIQUE NAMES: ",len(unique_name_list))
        return data_batch
...
class PointTransFeat(nn.Module):
    ...
    def forward(self, x):
        n_pts = x.size()[2]
        #print("PointTransFeat fwd n_pts: ",n_pts)
        if self.rotate:
            trans = self.stn(x)
        ...
        #print("PointTransFeat fwd x shape: ",x.shape)
    ...
    def feature_transform_regularizer(self, trans):
        ...
        if trans.is_cuda:
            ...
            I = I.cuda()
        #else:
            #print("Trans is not cuda")
        loss = torch.mean(torch.norm(torch.bmm(trans, trans.transpose(2, 1)) - I, dim=(1, 2)))
        #print("PointTransFeat loss: ",loss)
        return loss
...
class NNR(nn.Module):
    ...
    def __init__(self, input_dim, n_hidden, n_layer=6, cuda=True, p_rotate=False, feat_trans=False):
        ...
        self.enc_l = nn.TransformerEncoderLayer(d_model=n_hidden, nhead=8) #EDIT BY ERIN nhead=8 in Leifer code with n_hidden in fDNC_train being 48!
        ### edit by erin 10 may 2024 to try to get weights for fig 1b
        #attention_weights = self.enc_l.state_dict()['self_attn.in_proj_weight']
        #print('ATTENTION WEIGHTS:',attention_weights)
        ###

        self.model = nn.TransformerEncoder(self.enc_l, n_layer)

        # EDIT BY ERIN 06/22/2023 change to cpu only since else isn't working for some reason
...
class NIT(nn.Module):
    ...
    def __init__(self, input_dim, n_hidden, n_layer=6, cuda=True, p_rotate=False, feat_trans=False):
        ...
        self.enc_l = nn.TransformerEncoderLayer(d_model=n_hidden, nhead=8) # nhead #EDIT BY ERIN nhead=8 in Leifer code with n_hidden in fDNC_train being 48!
        self.model = nn.TransformerEncoder(self.enc_l, n_layer)
        #print(self.model)#.layer[-1].weight)
        # EDIT BY ERIN 06/22/2023 changing to cpu only since else statement not working
        self.device = torch.device("cuda:0" if cuda else "cpu")
        #self.device = torch.device("cpu")
    ...
    def encode(self, pts_padded, pts_length):
        # pts_padded should be of (b, num_pts, 3)
        #pts_proj = self.h_projection(pts_padded)
        #print("NIT pts_padded.shape: ",pts_padded.shape)
        #print("NIT pts_padded.shape after transpose: ",pts_padded.transpose(2,1).shape)
        pts_proj = self.point_f(pts_padded.transpose(2, 1))
        pts_proj = pts_proj.transpose(2, 1)
        mask = self.generate_sent_masks(pts_proj, pts_length)
        # add the src_key_mask need to test.
        ...
        return pts_encode.transpose(dim0=0, dim1=1)
    '''
    def encode_first_layer(self,pts_padded,pts_length):
        pts_proj = self.point_f(pts_padded.transpose(2, 1))
        pts_proj = pts_proj.transpose(2, 1)
        mask = self.generate_sent_masks(pts_proj, pts_length)
        # add the src_key_mask need to test.
        pts_first_encode = self.enc_l(pts_proj.transpose(dim0=0, dim1=1), src_key_padding_mask=mask)
        return pts_first_encode.transpose(dim0=0,dim1=1)
    '''
    ...
    def forward(self, pts, match_dict=None, ref_idx=0, mode='train'):
        ...
        return loss_dict, output_pairs
    '''
    # edit erin 13 may 2024 to try to get attention matrix for vis (/would possibly be interesting for behavior probing...)
    def forward_first_layer(self,pts,match_dict=None,ref_idx=0,mode='train'):
        pts_lengths = [len(s) for s in pts]
        # pts_padded should be of dimension (
        pts_padded = self.to_input_tensor(pts)

        pts_encode = self.encode_first_layer(pts_padded, pts_lengths)

        key_mtx = output[0]['key']
        value_mtx = output[0]['value']
        sim_mtx = torch.matmul(key_mtx,value_mtx.transpose(-2,-1))
        return sim_mtx
    '''

    def to_input_tensor(self, pts, pad_pt=[0, 0, 0]):
        sents_padded = []
        # commented back 25Aug...
        max_len = max(len(s) for s in pts) #orig line - not working b/c we're using 12 cells each - use line below
        #max_len = 12 # FIX MAKE READABLE BY FILE (len(s[0]) for s in pts)
        for s in pts:
            # TROUBLESHOOTING erin
            #print(len(s)) # these all print 12 i think
            #print(s)
            padded = [pad_pt] * max_len #UNCOMMENT
            # TROUBLESHOOTING Erin 7/5 remove print statements below
            #print("s: ",s,"\n")
            #print("padded: ",padded,"\n")
            padded[:len(s)] = s #UNCOMMENT
            sents_padded.append(padded) #UNCOMMENT
            #print("sents padded: ",sents_padded,"\n")
            # why 2 statments here?
        #sents_var = torch.tensor(sents_padded, dtype=torch.long, device=self.device)
        sents_var = torch.tensor(sents_padded, dtype=torch.float, device=self.device)
        return sents_var #torch.t(sents_var)
...
class NIT_Registration(NIT):
    """ Neuron Id transformer for registration between two datasets:
        - Transformer
    """
    # EDIT BY ERIN originally cuda=True! set to false 7/5
    def __init__(self, input_dim, n_hidden, n_layer=6, cuda=True, p_rotate=False, feat_trans=False):
    ...
    def encode(self, pts_padded, pts_length, ref_idx):
         # TROUBLESHOOTING by Erin 7/5 - change back to line pts_proj = self.point_f(pts_padded.transpose(2, 1))
         #print("pts padded (encode): ",pts_padded.shape, "\n")
         #print("pts_padded encode shape:",pts_padded.shape)
         #print("pts_padded upon reg transpose: ",pts_padded.transpose())
         #print("size of pts_padded upon reg transpose: ",pts_padded.transpose().shape)
         #print("pts_padded upon 2,1 transpose: ",pts_padded.transpose(2, 1))
         #print("size of pts_padded upon 2,1 transpose: ",pts_padded.transpose(2, 1).shape)
    
         # print("NIT Reg pts_padded.shape: ",pts_padded.shape) # This line is commented out by Ruohan on 8 Oct
         # print("NIT Reg pts_padded.shape after transpose: ",pts_padded.transpose(2,1).shape) # This line is commented out by Ruohan on 8 Oct
         pts_proj = self.point_f(pts_padded.transpose(2, 1))
         pts_proj = pts_proj.transpose(2, 1)
    ...
    def forward(self, pts, match_dict=None, ref_idx=0, mode='train'):
        ...
        embeddings['all']=pts_encode
        #print("pts_encode shape:", pts_encode.size())
        ref_emb = pts_encode[:, :pts_lengths[ref_idx], :]
        #print("ref emb shape:",ref_emb.size())
        mov_emb = pts_encode[:, pts_lengths[ref_idx]:, :]
        # the ref embryo encoding
        embeddings['ref_emb']=ref_emb
        # the test embryo encoding
        embeddings['mov_emb']=mov_emb
        ...
        return embeddings, loss_dict, output_pairs
